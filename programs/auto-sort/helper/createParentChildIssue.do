capture program drop createParentChildIssue
program define createParentChildIssue
syntax , 					/// 
	issuesFile(string) 		/// full path to issue file
	parentRoster(string) 	/// parent file, full file path
	parentId(string) 		/// item ID in parent file
	[parentCondit(string)] 	/// condition for flag value of 1 in parent file
	childRoster(string) 	/// child file, full file path
	childId(string) 		/// item ID in child file
	[childCondit(string)] 	/// condition for flag value of 1 in child file
	itemName(string) 	/// variable that contains item name, whether as string var or as labelled numeric
	[issueTypeForInt(integer 1)] 	/// error type at interview level
	[issueTypeForItem(integer 2)] 	/// error type at item level
	...
	* make item ID variable names match between parent and child
	...
	clonevar item_id = `parentId'
	tempfile parent
	save "`parent'"

	use "`childRoster'", clear
	clonevar item_id = `childId'
	tempfile child
	save "`child'"
	...
	* aggregate attributes to parent-item level
	clonevar item__id = item_id // to preserve this identifier in collapse
	collapse (max) parentFlag childFlag item_id (first) `itemName', ///
		by(interview__id interview__key item__id)
	...
	* create item-level issue
	if ("`issueTypeForItem'" != "" & "`issueCommForItem'" != "") {

		* capture roster indices
			local issueLocIDs "item__id"
		...
		* capture item name
		...
		// if item name comes from non-string
			...
			if ("`itemName'" == "`parentId'") {
				local itemName "item__id"
			}

			// if have label use it, otherwise cannot populate itemName
			local varLabel : value label `itemName'
			if ("`varLabel'" !=  "") {
		...
		if _rc != 0 {
			di "ERROR: `itemName' not successfully created"
			error 1
		}
		...
		* determine variables to keep based on variables created
		...
		if ("`issueVars'" != "") {
			local keepVars = "`keepVars' issueVars"
		}
	...
	* create interview-level issue
	if ("`issueTypeForInt'" != "" & "`issueCommForInt'" != "") {
		...
		* restore conflict data set from earlier
		use "`parentChildConflicts'", clear 

		* construct issue by populating observation with function inputs
		gen issueType 		= `issueTypeForInt'
		gen issueDesc 		= "`issueDesc'"
		gen issueComment 	= "`issueCommForInt'"			
		if ("`issueVars'") != "" {
			gen issueVars 		= "`issueVars'"
		}

		* add issues to error file
		duplicates drop interview__id interview__key issueType issueDesc issueComment issueVars, force
