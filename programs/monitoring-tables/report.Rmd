...
params:
 hhold_file: ""
 attempt_list_var: "tentative_appel__"
 interview_result: "result"
 attempt_file: ""
 attempt_time: "heure_debut" 
 answered_var: "reponse"
 talk_var: "parle_avec_membre"
...
attempts_stats <- compute_attempt_stats(
    ...
    overall_txt = "All interviewers")

attempts_stats %>% 
gt(rowname_col = "interviewer") %>% 
tab_header(title = "Contact attempts by interviewer") %>%
cols_label(
    num_assigned = html("Assigned<br>(N)"), 
    perc_attempted = html("Attempted<br>(%)"), 
    perc_no_attempted = html("Not attempted<br>(%)")) %>% 
fmt_number(
    columns = starts_with("perc"), 
    scale_by = 100) %>%
fmt_missing(columns = everything()) %>%
tab_footnote(
    footnote = "Assignments that are either not started or started but not synched.", 
    locations = cells_column_labels(
        columns = vars(perc_no_attempted))) %>%
tab_footnote(
    footnote = "Assignments received on the tablet only",
    locations = cells_column_labels(
...
contact_stats %>%
gt(rowname_col = "interviewer") %>%
tab_header(title = "Contact rate by interviewer") %>%
cols_label(
    num_attempts = html("Attempted<br>(N)"),
    perc_reached = html("Reached<br>(%)"),
    perc_not_reached = html("Not reached<br>(%)"),
    perc_unreachable = html("Unreachable<br>(%)")
) %>%
tab_footnote(
    footnote = "Any attempt resulted in successful contact",
    locations = cells_column_labels(columns = vars(perc_reached))
) %>%
tab_footnote(
    footnote = "Any attempt resulted in non-contact that may indicate possibility of continued contact attempts",
    locations = cells_column_labels(columns = vars(perc_not_reached))
) %>%
tab_footnote(
    footnote = "All attempts resulted in non-contact outcome where no further contacts can be profitably made.",
...
TODO: Revise qnr to collect info
...
conversion_stats %>%
gt(rowname_col = "interviewer") %>%
tab_header(title = "Conversion rate by interviewer") %>%
cols_label(
    num_reached = html("Reached<br>(N)"),
    perc_agreed = html("Agreed<br>(%)"),
    perc_refused = html("Refused<br>(%)")
) %>%
fmt_number(
    columns = starts_with("perc_"),
    scale_by = 100
...
tab_footnote(
    footnote = "Includes agreement to participate either now or a time proposed by the respondent", 
    locations = cells_column_labels(
...
length_stats %>%
gt(rowname_col = "interviewer") %>%
tab_header(title = "Length of completed interviews by interviewer") %>%
cols_label(
    num_obs = html("Interviews<br>(N)"),
    minimum = "Min",
    lower_hinge = "Q1",
    median = "Med",
    upper_hinge = "Q3",
...
attempt_time_stats_overall <- full_join(
    ...
    mutate_at(
        ...
        false = NA_real_,
        missing = NA_real_)) %>%
    mutate(interviewer = "All interviewers")
